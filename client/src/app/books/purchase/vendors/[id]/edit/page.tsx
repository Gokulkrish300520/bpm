...
export default function EditVendorPage() {
  ...
  const [vendorType, setVendorType] = useState<Vendor["type"]>("business");
  ...
  function onDocumentSelected(e: ChangeEvent<HTMLInputElement>) {
    const files = Array.from(e.target.files || []);
    const allowedFiles = files.filter(f => f.size <= 10 * 1024 * 1024).slice(0, 10);
    const metas = allowedFiles.map(f => ({ name: f.name, size: f.size, type: f.type }));
